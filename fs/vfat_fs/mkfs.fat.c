...
static void get_list_blocks(char *filename)
{
    ...
    while (1) {
	lineno++;
	ssize_t length = get_line(&line, &linesize, listfile);
	if (length < 0) {
...
static void setup_tables(void)
{
	...
	printf("%s has %d head%s and %d sector%s per track,\n",
	       device_name, letoh16(bs.heads),
	       (letoh16(bs.heads) != 1) ? "s" : "", letoh16(bs.secs_track),
	       (letoh16(bs.secs_track) != 1) ? "s" : "");
...
static void write_tables(void)
{
	...
	letoh32(bs.fat32.fat32_length) : letoh16(bs.fat_length);
    ...
    if (size_fat == 32) {
	seekto(letoh16(bs.fat32.info_sector) * sector_size, "info sector");
...
{
	if(!__pathimage || !__label || __size <= 0)return 1;
    // declare a specified vars
    FILE *msgfile;
    ...
    check_atari();
	// set volume label
	// for the volume label cant greater than 11 sizes!
	sprintf(volume_name, "%-11.11s", __label);
	if (memcmp(volume_name, "           ", MSDOS_NAME) == 0)
	    memcpy(volume_name, NO_NAME, MSDOS_NAME);
	// Verbose ? 1 TRUE
	// Set device, checkblocks and blocks size 
	verbose = shouldVerbose;
    device_name = __pathimage;
	blocks_specified = 1;
	blocks = __size;
	check = checkBlocksAfter;
	check_mount(device_name);	/* Is the device already mounted? */
	dev = open(device_name, O_EXCL | O_RDWR);	/* Is it a suitable device to build the FS on? */
	if (dev < 0) {
	    fprintf(stderr, "unable to open %s: %s\n", device_name, strerror(errno));
	    return 1;		/* The error exit code is 1! */
	}

    if (get_device_info(dev, &devinfo) < 0)
		return dies("error collecting information about %s", device_name);

    if (devinfo.size <= 0)
		return dies("unable to discover size of %s", device_name);
    ...
    if (!ignore_full_disk && devinfo.type == TYPE_FIXED &&
	    devinfo.partition == 0)
			return dies("Device partition expected, not making filesystem on entire device '%s' (use -I to override)", device_name);

    if (!ignore_full_disk && devinfo.has_children > 0)
		return dies("Partitions or virtual mappings on device '%s', not making filesystem (use -I to override)", device_name);
    establish_params(&devinfo); /* Establish the media parameters */
